...
### 谁管谁的回收：三区管理机制对照（ARC 只针对堆）

**ARC 只管堆上对象；栈靠编译器的栈帧机制自动回收，无需计数**：

```
内存区域             谁管理                机制
─────────     ─────────────        ──────────────
栈            编译器（静态）         栈帧自动回收：作用域/函数返回即失效，零开销
堆（ObjC 对象） ARC / MRC           引用计数：retain +1，release -1，归零释放
堆（C 内存）    malloc/free          手动管理，ARC 不管
全局/常量区     程序生命周期          永不释放
```

**ARC 到底管了什么**：不是管"栈上的存储"，而是管**「栈上指针 ↔ 堆上对象」之间的引用关系**：

```objc
- (void)foo {
    NSObject *obj = [[NSObject alloc] init];  // 堆对象，计数 1
    // ...
}   // ① 栈帧回收：obj 这个指针变量本身自动消失（无需计数）
    // ② ARC 在出作用域前插 release → 堆对象计数 0 → 释放
```

- **栈上的东西**（指针变量、int、struct、栈 Block）：编译器在**编译期**就安排好了——作用域结束回收，运行时零成本（只是移动栈指针）
- **堆上的对象**：编译器不知道何时没人要，只能靠**运行时计数**——ARC 本质就是在正确位置插入 retain/release 指令

**为什么栈不需要计数**：

```
栈 = 「自动清理的便签本」：进入作用域就展开，退出就翻过去，无需记账
堆 = 「共享仓库」：哪个标签（指针）都可能是最后一个，必须数人头（引用计数）
```
...
| 说法 | 正误 |
|------|------|
| ARC 管栈上变量的分配/回收 | ❌ 栈回收是编译器的栈帧机制，与 ARC 无关 |
| ARC 管栈上指针指向堆对象的生命周期 | ✅ 这才是 ARC 的职责 |
| 栈指针出作用域 = 堆对象释放 | ✅（ARC 下也因此触发 release） |
| C malloc 的内存由 ARC 管 | ❌ 照旧手动 `free` |
| `__weak` / autorelease | ✅ 都是引用计数机制的变体 |

> **一句话面试版**：ARC 针对**堆上对象**的 retain/release 计数，插入指令由编译器完成；栈上的存储由**栈帧机制**自动回收（作用域结束即失效，零计数零开销）；「栈上变量出作用域自动失效」和「堆对象靠计数释放」是**两套独立机制**。
